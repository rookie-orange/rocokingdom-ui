...
export function ExampleSection({ children, description, title }: ExampleSectionProps) {
  return (
    <section className="grid gap-5">
      <div>
        <p className="text-sm font-black text-primary">Example</p>
...
export function PreviewSurface({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-56 rounded-lg border border-stone/15 bg-[linear-gradient(135deg,rgb(255_255_255_/_0.54),rgb(255_198_95_/_0.14))] p-6 shadow-[0_14px_0_rgb(36_38_40_/_0.08)] max-sm:p-4">
      {children}
    </div>
  )
}
    ...
    <main className="min-h-svh bg-paper text-on-paper">
      <header className="sticky top-0 z-30 flex min-h-20 items-center justify-between gap-6 border-b border-stone/15 bg-stone px-8 text-on-stone max-[900px]:flex-wrap max-[900px]:px-5 max-[900px]:py-4">
        ...
        >
          ...
          <Link
            className="shrink-0 rounded-lg bg-primary px-3 py-2 text-sm font-black text-on-primary transition hover:brightness-105"
            to="/examples"
          >
            示例总览
          </Link>
      ...
      <div className="grid grid-cols-[280px_minmax(0,1fr)] max-[980px]:block">
            ...
            {examples.map((example) => (
              <Link
                className={[
                  ...
                  example.slug === slug
                    ? 'bg-stone text-on-stone'
                    : 'text-stone/70 hover:bg-primary hover:text-on-primary',
                ].join(' ')}
                key={example.slug}
                to={example.path}
              >
        ...
        <article className="mx-auto w-full max-w-6xl px-10 py-14 max-[980px]:px-5 max-[980px]:py-10">
          <section className="grid gap-7 border-b border-stone/15 pb-10">
            <div>
              <p className="text-base font-black text-primary">Component Example</p>
          ...
          {code ? (
            <section className="border-t border-stone/15 pt-10">
              <div>
                <p className="text-sm font-black text-primary">Code</p>
                <h2 className="mt-2 font-roco text-3xl font-black leading-none text-on-paper">
                  最小用法
                </h2>
              </div>
              <pre className="mt-5 overflow-x-auto rounded-lg bg-stone p-5 text-sm font-bold leading-6 text-on-stone shadow-[0_10px_0_rgb(36_38_40_/_0.12)]">
                <code>{code}</code>
              </pre>
            </section>
          ) : null}
