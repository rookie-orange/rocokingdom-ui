...
test('uses radix select and the shared floating content motion', () => {
  expect(selectSource).toContain("import * as RadixSelect from '@radix-ui/react-select'")
  expect(selectSource).toContain("import { floatingContentClassName } from '../floating-content'")
  ...
  expect(selectSource).toContain('floatingContentClassName,')
  expect(selectCss).not.toContain('@keyframes rk-select-content-in')
  expect(selectCss).not.toContain('@keyframes rk-select-content-presence-out')
